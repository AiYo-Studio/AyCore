...
public class ModifyListener {
     ...
     * @param files   监听文件
     */
    public static void addListener(JavaPlugin plugin, CustomExecute<ModifyData> execute, boolean async, File... files) {
     ...
     * @return 插件数据
     */
    public static PluginData registerPluginData(JavaPlugin plugin) {
        if (!AyCore.getInstance().dataMap.containsKey(plugin)) {
